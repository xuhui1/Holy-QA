...
# from ../utils.data-preprocess import context_question_text_preprocess
...
"""
max_context_length = 679  # 252
max_question_length = 40  # 14
...
idx_train_beg_array = idx_train_beg_array
...
context_lstm = Bidirectional(LSTM(units=dim_lstm_context//2, return_sequences=True))(context_emb) # (None, 300, 128)
...
# dense1 = Dense(500, activation='relu')(myLayer)
dense2 = Dense(units=max_context_length)(myLayer)
softmax_out = Activation('softmax')(dense2)

model = Model(inputs=[context_input, question_input], outputs=softmax_out)
model.summary()
...
# 权值，每层输出值的分布直方图
cbks = [tb_cb]
history = model.fit(x=[pad_txt_train_cnt, pad_txt_train_qst], y=idx_train_beg_array, epochs=10, batch_size=64,
            verbose=1, callbacks=cbks, validation_data=[[pad_txt_dev_cnt, pad_txt_dev_qst], idx_dev_beg_array])
model.save('squad_simple.h5', overwrite=True)
# """
# model evaluate
# """
